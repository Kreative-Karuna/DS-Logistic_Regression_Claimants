...
engine = create_engine("mysql+pymysql://{user}:{pw}@localhost/{db}"
                       .format(user = "root",    # user
                               pw = "WorkBench1",      # passwrd
                               db = "claims_db")) # database


# Load the offline data into Database to simulate client conditions
claims = pd.read_csv(r"C:\Users\Karuna Singh\Downloads\Mar.23\Deployment\logistic regresssion\claimants.csv").convert_dtypes()
claims.info()
...
# Load the new data
data = pd.read_excel(r"C:\Users\Karuna Singh\Downloads\Mar.23\Deployment\logistic regresssion\claims_test.xlsx").convert_dtypes()
